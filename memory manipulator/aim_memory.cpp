...
void get_process_memory(DWORD start, DWORD end) {
	...
	for (BYTE* addr = (BYTE*)sysInfo.lpMinimumApplicationAddress;
		...
		//if ((BYTE*)mem_info.BaseAddress - (BYTE*)0x700000000000 > 0) continue; // Skip module
		//if ((BYTE*)mem_info.BaseAddress - (BYTE*)0x10000000000 < 0) continue; // Skip stack
...
long long ptr2val(BYTE* ptr, BYTE type) {
	if (type == 1)
...
void detect(BYTE type, bool is_even) {
	...
	if (!filtered) {
		...
		if (choice[0] == "1")
			...
			for (DWORD i = 0; i < process_ptrs->size(); i++) {
				...
				for (DDWORD j = 0; j < block_size - bytes_to_read + 1; j++) {
					...
					prev_values[i][j / 2] = ptr2val(block_buffer + j, type);
					count++;
				}
				delete[] block_buffer;
			}
			...
			printf("2: increased [limit] / 3: decreased [limit] / 0: exit\n");
			string input; getline(cin, input, '\n');
			choice2 = parse(input);
			if (choice2[0] == "2" || choice2[0] == "3")
		...
		for (DWORD i = 0; i < process_ptrs->size(); i++) {
			...
			for (DDWORD j = 0; j < block_size - bytes_to_read + 1; j++) {
				...
				long long value = ptr2val(block_buffer + j, type);
				...
				if (choice[0] == "1" && value == target) {
					...
					if ((choice2[0] == "2" && value > prev_value && (value - prev_value) <= limit) ||
						(choice2[0] == "3" && value < prev_value && (prev_value - value) <= limit)) {
							filtered->push_back(Point(addr, value));
							count++;
					}
				}
	...
	// Keep filtering
	while (1) {
		printf("1 [target] / 2: increased [limit] / 3: decreased [limit] 4: same / 0: exit\n");
		string input; getline(cin, input, '\n');
		vector<string> choice = parse(input);
		if (choice[0] == "1")
			target = stoul(choice[1]);
		else if (choice[0] == "2" || choice[0] == "3")
			limit = stoull(choice[1]);
...
void main() {
	...
	while (true) {
		...
		printf("4: get result / 5 [ptr_addr]: read ptr / 6 [addr] [range]: read range\n");
		printf("7 [addr] [val]: write int / 8 [addr] [val]: write float / 9 [addr] [val]: write double 0 : exit\n");
		string input; getline(cin, input, '\n');
		vector<string> choice = parse(input);
		if (choice[0] == "1")
